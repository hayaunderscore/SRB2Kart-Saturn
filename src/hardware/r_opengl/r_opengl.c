...
EXPORT boolean HWRAPI(CompileShaders) (void)
{
	...
	for (i = 0; gl_shadersources[i].vertex && gl_shadersources[i].fragment; i++)
	{
		...
		{
			I_Error("CompileShaders: Error creating vertex shader %s\n", HWR_GetShaderName(i));
			continue;
		}
		...
		{
			I_Error("CompileShaders: Error creating fragment shader %s\n", HWR_GetShaderName(i));
			continue;
		}
		...
		if (result != GL_TRUE)
		{
			shader->program = 0;
			shader->custom = false;
			I_Error("CompileShaders: Error linking shader program %s\n", HWR_GetShaderName(i));
			continue;
		}
...
EXPORT void HWRAPI(RenderBatches) (precise_t *sSortTime, precise_t *sDrawTime, int *sNumPolys, int *sNumVerts, int *sNumCalls, int *sNumShaders, int *sNumTextures, int *sNumPolyFlags, int *sNumColors)
{
	...
	*sDrawTime = I_GetPreciseTime();
	...
	currentTexture = polygonArraySorted[0]->texNum;
